...
class JwtManager(context: Context) {
    ...
    companion object {
        private const val KEY_JWT = "jwt"; private const val KEY_PHONE = "phone"
    ...
    fun saveJwt(jwt: String, phone: String) {
        prefs.edit().putString(KEY_JWT, jwt).putString(KEY_PHONE, phone)
            .putLong(KEY_EXP, getExpiration(jwt)).apply()
    }
    fun saveProfile(alias: String, avatar: String = "", todusId: String = "") {
        prefs.edit().putString(KEY_ALIAS, alias).putString(KEY_AVATAR, avatar)
            .putString(KEY_TODUS_ID, todusId).apply()
    }
